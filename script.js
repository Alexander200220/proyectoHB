...
function seguirCursorConEstelaLuegoIrAlBoton() {
    ...
    const colores = ['#ff4d4d', '#ffa64d', '#ffff66', '#66ff66', '#66ccff', '#cc66ff'];
...
function desplegarHoja() {
  ...
  document.getElementById("correa").addEventListener("click", () => {
    ...
    // Aplica la clase de transformación a la hoja después de un pequeño retraso
    setTimeout(() => {
      hoja.classList.remove('hoja-inicio');
      hoja.classList.add('visible');
      escribirTextoLineaPorLinea();
    }, 500); // 500ms para permitir que el botón se desvanezca un poco
});
    ...
    function escribirSiguienteLinea() {
      ...
      function escribirCaracter() {
        if (charIndex < chars.length) {
          ...
          charIndex++;
          setTimeout(escribirCaracter, 10);
        } else {
          index++;
